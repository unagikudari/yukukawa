...
observation_method from the attested adapter
stale-write basis
...
If authority-bearing context resolves exactly one Project, it is filled automatically. If multiple remain plausible, return `needs_selection`.
...
`event_type`, `subject`, `project`, and links SHOULD be omitted from the actual call when authority-bearing Work/causation context determines them uniquely.
...
If Work uniquely binds the Plan and Finding, Kawa attaches subject, Project, `based_on`, Work basis, and attested identity automatically.
...
### 8.4 Attested fields are never caller authority
...
observation_method for attested deterministic collectors
...
The matching attested basis is attached automatically from exact Work/request context.
...
Opaque identifiers are generated by Kawa or an attested identity-mapping adapter, not by LLM creativity.
...
The caller cannot supply attested identity/provenance fields.
